    ...
    <div class="admin-shell">
        ...
        <main class="admin-content admin-content--form">
            ...
            <form method="POST" action="{{ route('admin.profile.settings.update') }}" class="admin-form" aria-labelledby="personal-data-heading">
                ...
                @include('admin.partials.help', ['title' => 'Изменение личных данных', 'text' => 'Имя отображается в панели управления, а электронная почта используется для входа. После смены почты входите с новым адресом. Телефон остаётся необязательным контактным полем.', 'items' => []])

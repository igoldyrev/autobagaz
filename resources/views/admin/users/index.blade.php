        ...
        <main class="admin-content admin-content--wide">
            <div class="page-heading">
                ...
                <div class="heading-actions">
                    <a class="button button--secondary button--inline" href="{{ route('admin.activity.index') }}">Журнал действий</a>
            ...
            <div class="table-wrap users-table-wrap">
                <table class="admin-table">
                    ...
                    <tbody>
                        @forelse ($users as $user)
                            <tr>
                                ...
                                <td class="table-secondary">
                                    {{ $user->last_login_at?->timezone(config('app.display_timezone'))->format('d.m.Y H:i') ?? 'Нет данных' }}
                                </td>
                                <td class="table-secondary">
                                    @if ($user->last_seen_at)
                                        {{ $user->last_seen_at->timezone(config('app.display_timezone'))->format('d.m.Y H:i') }}
                                        @if ($user->last_seen_at->gt(now()->subMinutes(10)))
                                            <br><span class="status status--active">Недавно активен</span>
                                        @endif
                                    @else
                                        Не зафиксирована
                                    @endif
                        ...
                        @empty
                            <tr><td class="empty-state" colspan="6">Пользователей пока нет.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
